...
1. 사이드바에서 **과목·사고 프레임** 선택 (일반/수학/과학/역사/국어·문학/사회)
2. 본문에서 다루고 싶은 문제 입력
3. AI가 답을 주지 않고 질문·반례·관점을 제시 → 응답 → 평가자가 단계 통과 여부 판단
4. 5단계를 모두 통과하면 비로소 종합 답변 제공
5. 사이드바에서 **Thinking Data 다운로드** 버튼으로 JSON 저장

모델은 `gpt-4o` 로 고정되어 있습니다 (instruction following 품질). 변경하려면 `app.py` 상단의 `MODEL` 상수 수정.
...
모델 `gpt-4o` 기준:
- 한 턴(코치 + 평가자 2회 호출) ≈ $0.01~0.03
- 한 세션(보통 6~8 user 턴 + 마지막 2단계 합성) ≈ $0.10~0.30
- 합성기는 세션당 1회만 (재합성 버튼 누르지 않는 한)
...
- 평가자 LLM이 매 턴 별도 호출되므로, 한 턴 ≈ 2회 호출입니다 (gpt-4o 기준).
  - 한 세션(6~8 user 턴) ≈ $0.10~0.30
  - 합성은 세션당 1회만 발생
